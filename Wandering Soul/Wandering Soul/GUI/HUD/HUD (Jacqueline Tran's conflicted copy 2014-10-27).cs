    ...
    public class HUD : GUI
    {
        ...
        public void Draw()
        {
            ...
            for (int p = 0; p < Program.Data.MyPlayerData[Program.CurrentSaveData].MainParty.MyParty.Count; p++)
            {
                ...
                t.DisplayedString = Program.Data.MyPlayerData[Program.CurrentSaveData].MainParty.MyParty[p].Name;
                t.Position = new Vector2f(X + 50, Y + (s.Texture.Size.Y + 10) * p - 1);
                _screen.Draw(t);

                t.DisplayedString = "Level: " + Program.Data.MyPlayerData[Program.CurrentSaveData].MainParty.MyParty[p].Level;
                t.Position = new Vector2f(X + 100, Y + (s.Texture.Size.Y + 10) * p + 38);
                _screen.Draw(t);

                t.DisplayedString = Enum.GetName(typeof(JobType),(JobType)Program.Data.MyPlayerData[Program.CurrentSaveData].MainParty.MyParty[p].Job);
